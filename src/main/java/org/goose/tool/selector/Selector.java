...
/**
 * 功能描述：最优子树选择器。 Copyright: Copyright (c) 2015 Company: 电子科技大学 Author: MaoKai
 * Version: 1.0 Time:2015/4/19 9:45
 */
public class Selector {
	...
	/**
	 * 通过停止词分析、分词技术、链接稠密度分析、加权求值等分析判断 计算出最优父节点。
	 * @param document
	 * @return
	 */
	...
	private Elements getAllNodesToCheck(Document document) {
		/*
		 * returns a list of nodes we want to search on like paragraphs and
		 * tables
		 */
	...
	private Elements calculateScoreAndUpdateParent(Elements nodes_to_check) throws IOException {
	...
	public boolean is_boostable(Element textNode) throws IOException {
		...
		for (Element previousSibling : previousSiblings) {
			if (!para.equals(previousSibling.tag().getName()))
				continue;
			if (steps_away > max_stepsaway_from_node)
				return false;
			String text = previousSibling.html();
			int count = caculateStopWords(text);
			if (count > minimum_stopword_count)
				return true;
			steps_away++;
		}
		return false;
